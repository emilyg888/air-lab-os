...
def load_policy(path: Path = POLICY_PATH) -> ScoringPolicy:
    ...
    policy = ScoringPolicy(
        primary_metric_weight=weights["primary_metric"],
        explainability_weight=weights["explainability"],
...
def _coerce_scoring_policy(
    ...
    coerced = ScoringPolicy(
        primary_metric_weight=weights["primary_metric"],
        explainability_weight=weights["explainability"],
        latency_weight=weights["latency"],
        cost_weight=weights["cost"],
        latency_max_ms=policy["latency"]["max_ms"],
        cost_max_per_1k=policy["cost"]["max_per_1k"],
...
def _apply_primary_metric_floor(score: float, primary_metric: float) -> float:
    if primary_metric < PRIMARY_METRIC_FLOOR:
        return 0.0
    return score
...
def _evaluate_handle_result(
    ...
    composite = _apply_primary_metric_floor(composite, primary_value)
...
def _evaluate_mapping_result(
    ...
    final_score = _apply_primary_metric_floor(final_score, primary_metric)
...
def evaluate(
    ...
    if isinstance(target, DatasetHandle):
        return _evaluate_handle_result(result, target, policy)

    if policy is None:
        raise ValueError("policy is required for dict-based evaluation")
    return _evaluate_mapping_result(result, target, policy)
